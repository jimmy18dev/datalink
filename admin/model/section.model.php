...
class SectionModel extends Database{
	public function create($name,$description,$redirect){
		parent::query('INSERT INTO RTH_Section(name,description,redirect,create_time,update_time) VALUE(:name,:description,:redirect,:create_time,:update_time)');
		parent::bind(':name', 			$name);
		parent::bind(':description', 	$description);
		parent::bind(':redirect', 		$redirect);
		parent::bind(':create_time',	date('Y-m-d H:i:s'));
		parent::bind(':update_time',	date('Y-m-d H:i:s'));
		parent::execute();
		return parent::lastInsertId();
	}
	public function edit($id,$name,$description,$redirect){
		parent::query('UPDATE RTH_Section SET name = :name, description = :description, redirect = :redirect, update_time = :update_time WHERE id = :id');
		parent::bind(':id', 			$id);
		parent::bind(':name', 			$name);
		parent::bind(':description', 	$description);
		parent::bind(':redirect', 		$redirect);
