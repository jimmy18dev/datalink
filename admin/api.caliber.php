			...
			switch ($_POST['action']) {
				case 'submit':
					if(true){
						...
						if(!empty($caliber_id) && isset($caliber_id)){
							$message = 'caliber edited';
							$caliber->editCaliber($caliber_id,$code,$family,$description,$stdtime);
						}else{
							$return_id = $caliber->createCaliber($code,$description,$family,$stdtime);
							$message = 'caliber created';
						}
		...
		case 'caliber':
			switch ($_GET['action']) {
				...
				case 'list_operation':
					...
					break;
				case 'get':
					$caliber_id = $_GET['caliber_id'];
					$dataset = $caliber->getCaliber($caliber_id);
					// Export data to json format
					$data = array(
						"apiVersion" => "1.0",
						"data" => array(
